...
보조지표(EMA, RSI, MACD, BB 등) 외에 트레이딩 패턴(EMA 조건 플래그, 커스텀 시그널 등)을
...
# patterns/custom_pattern.py — series 모드 (복잡한 패턴)
@register_pattern(name="sig_custom_pattern", priority=120, mode="series",
                  requires=["ema_112", "ema_224", "ema_448"])
def compute_custom_pattern(df: pl.DataFrame) -> pl.Series:
    # 전체 시계열 접근 필요
    ...
    return pl.Series("sig_custom_pattern", result_bool_list)
```
...
커스텀 패턴 시그널은 날짜 단위로는 계산 불가 — 과거 전체 시계열이 필요하기 때문.
...
- 커스텀 패턴 시그널이 날짜 파티션에도 자연스럽게 포함됨
...
```sql
-- 스크리닝: 3만+ 심볼 × 500일 데이터에서 특정 시그널 켜진 것만
SELECT symbol, date FROM silver
WHERE sig_custom_pattern = true AND date = '2026-06-17'
-- Parquet: 해당 컬럼만 읽음 (columnar I/O)
-- JSON: 모든 필드 파싱 후 필터
```
...
| 컬럼명 | 설명 |
|---|---|
| `sig_custom_pattern` | 커스텀 패턴 발생 여부 (예시) |
