...
`e:\MyBlog\quartz\deploy.bat` 더블클릭 → 커밋 메시지 입력 → 자동 push → 배포 완료.

- 이 파일이 `quartz` 저장소에 포함되어 있어 다른 PC에서 `Blog` 저장소를 clone하면 그대로 사용 가능.
- 스크립트가 절대경로를 쓰지 않고 자기 위치(`%~dp0`)를 기준으로 동작하므로 설치 경로가 달라도 실행 가능.
